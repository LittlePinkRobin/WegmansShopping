...
def update_list(session, name, sku):
    ...
    #conn = http.client.HTTPSConnection('api.amazonalexa.com', 443)
    #conn.set_debuglevel(5)
    #conn.request('POST', todo_path, payload, headers)
    conn = http.client.HTTPSConnection("api.amazonalexa.com", 443)

    payload = {"value": "my item","status": "completed"}
    p = str(json.dumps(payload))
    headers = {
    'Authorization': "Bearer " + token,
    'Content-Type': "application/json"
    }

    conn.request("POST", "v2/householdlists/%s/items" % todo_path, p, headers)
    response = conn.getresponse();
    status = response.status
    print(status)
    if status == 403:
